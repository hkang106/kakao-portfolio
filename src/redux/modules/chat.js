	...
	return (dispatch, getState) => {
      ...
      */
		...
		fetch('https://dialogue-data-character-backend.themusio.com/api/chat/jp/si/', {
			...
			.then((json) => {
				const { bot_response: { body, comment_id }, next_options } = json;
				const { chat: { is_option_selected, messages } } = getState();
...
export default handleActions(
	{
		...
		[SET_BOT_RESPONSE]: (state, action) => {
			const { payload: { body, comment_id } } = action;
		...
		[SET_OPTION]: (state, action) => {
			const { payload: { next_options } } = action;
			let ids = [];
			let opts = [];
		...
		[CLICK_OPTION]: (state, action) => {
			const { payload: { message, selected_id } } = action;
			return {
				...state,
